...
static int lm363x_regulator_probe(struct platform_device *pdev)
{
	...
	cfg.regmap = regmap;
	cfg.of_node = lm363x_regulator_matches[id].of_node;

	/*
	 * LDOs of LM3627X and LM3632 can be controlled by external pin.
	...
	if (gpio_is_valid(ena_gpio)) {
		...
		if (cfg.init_data && cfg.init_data->constraints.boot_on)
