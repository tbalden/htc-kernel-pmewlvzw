...
Filename : anx7688_public_interface.c
...
/**
 * @desc: The Interface that AP sends the specific USB PD command to anx7688
 *
 * @param:
...
/**
 * @desc:   The Interface that AP handle the specific USB PD command from anx7688
 *
 * @param:
 *	type: PD message type, define enum PD_MSG_TYPE.
...
u8 dispatch_rcvd_pd_msg(PD_MSG_TYPE type, void *para, u8 para_len)
{
    ...
    case TYPE_PWR_SRC_CAP:
        ...
        WriteReg(VBUS_DELAY_TIME, 0xa0);  //vbus off  delay time (adjust it as your plaform)
#endif
        downstream_pd_cap = 1;
    ...
    case TYPE_PSWAP_REQ:
		...
		if (anx7688_get_prop(ANX_PROLE) == PR_SINK) {
			anx7688_set_prop(ANX_PROLE_CHANGE, SNK_TO_SRC);
			pr_info("set pr_change\n");
		}
		else if (anx7688_get_prop(ANX_PROLE) == PR_SOURCE) {
			anx7688_set_prop(ANX_PROLE_CHANGE, SRC_TO_SNK);
			pr_info("set pr_change\n");
		}
