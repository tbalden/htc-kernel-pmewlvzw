...
#define HIGH_SPEED_SIGNAL_RATE_LIMIT            (25 * 65536 / 100) /* 0.40 * 65536 */
...
static void stmvl53l0_ps_read_measurement(struct stmvl53l0_data *data)
{
    ...
    data->rangeData.RangeStatus = RangeStatus_determination(data->rangeData.RangeStatus);
        ...
        uint8_t scaling, init_mode_e mode)
{
    ...
    /* Enable laser component via io extender gpio
     * (power-down pin) */
    gpio_set_value(data->pwdn_gpio, 1);
    msleep(2);
#endif
...
static int stmvl53l0_stop(struct stmvl53l0_data *data)
{
    ...
    if(!gpio_is_valid(data->power_gpio)) {
        ...
        vl53l0_errmsg("Don't turn off power_gpio\n");
    }
    udelay(500);
