...
{
	uint8_t val_high=255, val_low=255, ID0=0, ID1=0;
	uint8_t sensor_id;
	char data[3];
...
static int hx_sensitivity_setup(struct himax_ts_data *cs, uint8_t level)
{
	...
	if (err < 0)
		E("%s: I2C write fail. err %d, addr: 0x%02X.\n", __func__, err, CS_SENSITIVITY_START_REG);
