...
#define t_pr_info(fmt, args...)		printk(KERN_INFO "[TP] " fmt,  ##args)
#define t_pr_noti(fmt, args...)		printk(KERN_NOTICE "[TP] " fmt,  ##args)
#define t_pr_warn(fmt, args...)		printk(KERN_WARNING "[TP] " fmt,  ##args)
#define t_pr_err(fmt, args...)		printk(KERN_ERR "[TP][ERR] %s: " fmt, __func__, ##args)
...
#define __t_dev_info(_dev, fmt, args...)	\
		do {								\
			if (_dev != NULL) {					\
				struct siw_ts *_ts = to_touch_core(_dev);	\
				if(_ts == NULL)					\
					__t_dev_none();				\
				else if (_ts->debug_mask > 0)			\
					t_pr_info(fmt, ##args);			\
			}							\
		} while (0)
...
#define __t_dev_warn(_dev, fmt, args...)	\
		do {				\
			if (_dev != NULL)	\
				t_pr_warn(fmt, ##args);	\
		} while (0)

#define __t_dev_err(_dev, fmt, args...)	\
		do {				\
			if (_dev != NULL)	\
				t_pr_err(fmt, ##args);	\
		} while (0)
