...
static void __ext_watch_font_download(struct siw_touch_chip *chip)
{
	struct siw_ts *ts = chip->ts;
...
static void ext_watch_font_download(struct work_struct *font_download_work)
{
	...
	__ext_watch_font_download(chip);
}
...
static void ext_watch_font_download_func(const struct firmware *fw, void *context)
{
	struct siw_ts *ts = (struct siw_ts *)context;
	struct siw_touch_chip *chip = to_touch_chip(ts->dev);
	struct watch_data *watch = chip->watch;

	if (IS_ERR_OR_NULL(fw)) {
		t_watch_err(ts->dev, "no font image\n");
		return;
	}

	memset(watch->ext_wdata.font_data, 0, watch->font_max_size);
	watch->font_written_size = (fw->size < watch->font_max_size) ?
					(u32)fw->size : watch->font_max_size;
	if (fw->data != NULL)
		memcpy(watch->ext_wdata.font_data, (u8 *)fw->data, watch->font_written_size);

	atomic_set(&watch->state.font_status, FONT_DOWNLOADING);

	t_watch_info(ts->dev,
			"%s: font image preload done (%d, %s)\n", __func__,
			watch->font_written_size, ts->watch_font_image);
	t_watch_info(ts->dev,
			" %02X %02X %02X %02X"
			" %02X %02X %02X %02X"
			" ...\n",
			watch->ext_wdata.font_data[0], watch->ext_wdata.font_data[1],
			watch->ext_wdata.font_data[2], watch->ext_wdata.font_data[3],
			watch->ext_wdata.font_data[4], watch->ext_wdata.font_data[5],
			watch->ext_wdata.font_data[6], watch->ext_wdata.font_data[7]);
...
out:
	filp_close(filp, 0);
#else
	if (watch->font_written_size > 0) {	/* After probe and trigged by sysfs attribute command */
		ret = request_firmware(&fw, name, dev);

		if (ret < 0) {
			if (ret == -ENOENT) {
				t_dev_err(dev, "can't find font bin: %s\n", name);
			} else {
				t_dev_err(dev, "can't request fw: %s, %d\n", name, ret);
			}
		} else {
			memset(buf, 0, max_size);
			ret = (fw->size < max_size) ? (int)fw->size : max_size;
			if (fw->data != NULL)
				memcpy(buf, (u8 *)fw->data, ret);
		}
	} else if (ts->skip_upgrade) {
		t_watch_info(dev, "Skip touch font download.\n");
		ret = -EPERM;

	} else {	/* During probe */
		ret = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				name, dev, GFP_KERNEL, ts , ext_watch_font_download_func);
		if (ret == 0) {
			t_watch_info(dev, "request_firmware_nowait\n");
			ret = -EAGAIN;
		}
	}
#endif
...
ssize_t store_ext_watch_font_onoff(struct device *dev,
					const char *buf, size_t count)
{
	struct siw_touch_chip *chip = to_touch_chip(dev);
	struct siw_ts *ts = chip->ts;
...
ssize_t store_ext_watch_config_time_sync(struct device *dev,
					const char *buf, size_t count)
{
	struct siw_touch_chip *chip = to_touch_chip(dev);
	struct siw_ts *ts = chip->ts;
...
void siw_hal_watch_font_update(struct device *dev)
{
	struct siw_touch_chip *chip = to_touch_chip(dev);
	struct siw_ts *ts = chip->ts;
	struct siw_hal_reg *reg = chip->reg;
	...
	//check crc
	ret = siw_hal_read_value(dev, reg->tc_status, &status);
	if (ret < 0) {
		t_watch_err(dev, "read status failed\n");
		return;
	}
	if (!(status & FONT_MEM_CRC)) {
		t_watch_info(dev, "crc fail [tc_status %08Xh]\n", status);
		status = 1;
	} else
		t_watch_info(dev, "crc ok\n");

	if ((ts->enable_clock_setting[2] < ts->def_font_cnt)
			&& ((ts->font_index != ts->enable_clock_setting[2]) || (status == 1))) {

		ts->watch_font_image = ts->watch_font_image_list[ts->enable_clock_setting[2]];
		t_watch_info(dev, "font_image: %s\n", ts->watch_font_image);
		for (i = 1; i < 51; i++) {
			ret = ext_watch_fontdata_preload(dev);
			if (ret) {
				t_watch_info(dev, "read font data: trial #%d, ret = %d\n", i, ret);
				mdelay(10);
			} else
				break;
		}
		if ((ret == 0) && (atomic_read(&watch->state.font_status) == FONT_DOWNLOADING)) {
			//download font data, max retry 3
			for (i = 1; i < 4; i++) {
				__ext_watch_font_download(chip);

				if (atomic_read(&watch->state.font_status) == FONT_READY) {
					//check font crc, max retry 3
					mdelay(20);
					for (j = 1; j < 4; j++) {
						ret = siw_hal_read_value(dev, reg->tc_status, &status);
						if (ret < 0) {
						...
						if (!(status & FONT_MEM_CRC)) {
							...
							t_watch_info(dev, "crc ok\n");
							ts->font_index = ts->enable_clock_setting[2];
							goto font_update_out;
						}
					}
				} else {
					t_watch_err(dev, "update font image: write failed\n");
					goto font_update_out;
				}

				t_watch_err(dev, "update font image: trial #%d\n", i);
			}
		} else
			t_watch_err(dev, "read font data failed/improper font status\n");
	}
...
void siw_hal_watch_font_onoff(struct device *dev, bool enable)
{
	char *watch_cmd;

	watch_cmd = kzalloc(sizeof(char) * 3, GFP_KERNEL);
	if (watch_cmd) {
		if (enable) {
			scnprintf(watch_cmd, 3, "d\n");
			store_ext_watch_config_time_sync(dev, watch_cmd, 2);
		}
		scnprintf(watch_cmd, 3, "%d\n", enable);
		store_ext_watch_font_onoff(dev, watch_cmd, 2);

		kfree(watch_cmd);
	}
}

int siw_hal_watch_update_font_effect_24h(struct device *dev, int hour)
{
	struct siw_touch_chip *chip = to_touch_chip(dev);
	struct watch_data *watch = (struct watch_data *)chip->watch;
	struct ext_watch_cfg_position *position = &watch->ext_wdata.position;
	...
	if (hour == 24 || hour == 12) {
		if ((position->h24_en != (hour==24 ? 1 : 0))
				|| (position->midnight_hour_zero_en != (hour==24 ? 1 : 0))) {
			position->h24_en = (hour==24 ? 1 : 0);
			position->midnight_hour_zero_en = (hour==24 ? 1 : 0);
	...
	return 0;// invalid or the same value
}
...
void siw_hal_watch_set_disp_waton(struct device *dev, bool value)
{
	struct siw_touch_chip *chip = to_touch_chip(dev);
	struct watch_data *watch = (struct watch_data *)chip->watch;
