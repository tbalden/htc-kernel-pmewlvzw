					...
					char *fname_new)
{
	...
	strlcpy(pathname_new, pathname, strlen(pathname));
...
ssize_t prd_show_delta(struct device *dev, char *buf)
{
...
ssize_t prd_show_rawdata_prd(struct device *dev, char *buf)
{
...
#define __TOUCH_PRD_PERM	(S_IRUGO | S_IWUSR)
#else
