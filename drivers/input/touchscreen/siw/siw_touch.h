...
static const char aod_mode_str[FB_AOD_MAX - FB_AOD_PREPARE_OFF][5] = {
	"OffP",
	"Off",
	"U0",
	"U2",
...
struct touch_feature_ctrl {
	int aod_mode;
	int prev_aod_mode;
	bool tp_bus_sel_en;
	bool bus_to_mcu;
#define SWITCH_DELAY_TIME_MAX 50	//ms
	int switch_delay_time[FB_AOD_MAX];
	bool charger_setup_skip;
...
//	uint32_t cover_on_command;
//	uint32_t cover_off_command;
#endif
...
#define NANOHUB_I2C_DELAY 5
#define NANOHUB_I2C_MAX_WAIT_COUNT 60
#endif
...
//	struct platform_device *pdev;
	...
	uint32_t debug_reg[3]; //0x8DB, 0x8DD, 0x8E0
	uint32_t eng_id_mask;
	uint32_t eng_id;
	bool u3_disp_setting;
	bool skip_upgrade;	//skip both firmware & font data download
#endif
	struct siw_touch_pdata *pdata;
	...
	struct touch_feature_ctrl mode_ctrl;
...
#define _FORCE_FWUP_SYS_STORE	(1<<3)
	...
	/* __SIW_SUPPORT_WATCH */
#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG)
#define MAX_FONT_IMAGE_COUNT 4
	int def_font_cnt;
	uint8_t font_index;
	uint8_t enable_clock_setting[3];
	const char *watch_font_image_list[MAX_FONT_IMAGE_COUNT];
#endif
...
#if defined(__SIW_CONFIG_FB)
#if 0
	struct semaphore switch_sensor_hub_sema;
#endif
...
#endif	/* __SIW_ATTR_PERMISSION_ALL */
...
#ifdef CONFIG_NANOHUB_TP_SWITCH
	pins->switch_pin = pins_src->switch_pin;
#endif
...
#ifdef CONFIG_NANOHUB_TP_SWITCH
static inline int touch_switch_pin(struct siw_ts *ts)
{
	return ts->pins.switch_pin;
}
#endif
...
extern void siw_hal_watch_font_update(struct device *dev);
extern void siw_hal_watch_font_onoff(struct device *dev, bool enable);
...
#if !defined(MODULE) && !defined(CONFIG_TOUCHSCREEN_HTC_DEBUG)
...
#define siw_chip_module_init(_name, _data, _desc, _author)	\
		static void __init chip_##_name##_driver_init_async(void *unused, async_cookie_t cookie)\
		{	\
			touch_msleep(200);	\
			t_pr_info("%s_async: ++\n", _data.pdata->drv_name);	\
			siw_touch_bus_add_driver(&_data);	\
			t_pr_info("%s_async: --\n", _data.pdata->drv_name);	\
		}	\
		...
		{	\
			...
			async_schedule(chip_##_name##_driver_init_async, NULL);	\
			t_pr_info("%s: %s driver init exit\n",	\
				_data.pdata->drv_name, _name);	\
			return 0;	\
		}	\
		...
		MODULE_LICENSE("GPL");
#else /* CONFIG_TOUCHSCREEN_HTC_DEBUG */
#define siw_chip_module_init(_name, _data, _desc, _author)	\
		static int __init chip_##_name##_driver_init(void)\
		{	\
			touch_msleep(200);	\
