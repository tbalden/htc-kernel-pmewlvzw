...
ssize_t _store_irq_state(struct device *dev,
				const char *buf, size_t count)
{
	struct siw_touch_chip *chip = to_touch_chip(dev);
	struct siw_ts *ts = chip->ts;
...
ssize_t _store_glove_state(struct device *dev,
				const char *buf, size_t count)
{
	struct siw_ts *ts = to_touch_core(dev);
	int value = 0;
...
extern int siw_touch_init_sysfs_htc(struct siw_ts *ts);
extern void siw_touch_free_sysfs_htc(struct siw_ts *ts);
