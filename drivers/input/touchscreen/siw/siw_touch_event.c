	...
	for (i = 0; i < touch_max_finger(ts); i++) {
		if (old_mask & (1 << i)) {
			...
			if (atomic_read(&ts->state.glove))
				input_report_abs(ts->input, ABS_MT_GLOVE, 1);
			else
				input_report_abs(ts->input, ABS_MT_GLOVE, 0);
#endif
...
#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG)
static void siw_touch_report_debug(struct device *dev, u32 *data)
{
	u16 reg_addr[3] = {0x8db, 0x8dd, 0x8e0};
	int ret = 0, ind;
	...
	for(ind = 0; ind < 3; ind++) {
		ret = siw_hal_read_value(dev,
					...
					&data[ind]);
		if (ret < 0)
			t_dev_err(dev, "rd: reg[0x%04X] fail ret = %d\n", reg_addr[ind], ret);
	}
}

#define TOOL_TYPE_GLOVE		6
#define TOOL_TYPE_FINGER	13
static char state2char(uint8_t status)
{
	switch (status) {
		case TOOL_TYPE_FINGER:
			return 'F';
		case TOOL_TYPE_GLOVE:
			return 'G';
		default:
			return 'F';
	}
...
void siw_touch_report_event(void *ts_data)
{
	...
	int i;
#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG)
	u32 debug_reg[3]; //0x8DB, 0x8DD, 0x8E0
#endif
...
//	t_dev_trcf(idev);
	...
	if (ts->debug_mask & (TOUCH_KPI_LOG | TOUCH_BREAKDOWN_TIME)) {
		...
		if (ts->tdata[0].y > 0)
			trace_clock_set_rate("tp_report", ts->tdata[0].y - 160, -4);
	}
	if (ts->debug_mask & TOUCH_BREAKDOWN_LOG)
		pr_info("[TP][KPI] %s: input_sync done\n", __func__);

	if (ts->debug_mask == 0) {
		siw_touch_report_debug(ts->dev, &debug_reg[0]);
		if (ts->debug_reg[2] != debug_reg[2]) {
			ts->debug_reg[0] = debug_reg[0];
			ts->debug_reg[1] = debug_reg[1];
			ts->debug_reg[2] = debug_reg[2];
			pr_info(" thonum=x%08X, diffsum=x%08X, status=x%08X\n",
					ts->debug_reg[0], ts->debug_reg[1], ts->debug_reg[2]);
		}
	} else if (ts->debug_mask & TOUCH_DOWN_UP_LOG) {
		for (i = 0; i < touch_max_finger(ts); i++) {
			if (new_mask & (1 << i)) {
				if (press_mask & (1 << i)) {
					siw_touch_report_debug(ts->dev, &ts->debug_reg[0]);
					pr_info(" Screen:%c[%02d/%02d]:%s, X=%d, Y=%d, Z=%d, thonum=x%08X, diffsum=x%08X, status=x%08X\n",
							state2char(ts->tdata[i].type),
							i,
							ts->tcount,
							"Down",
							ts->tdata[i].x,
					...
					siw_touch_report_debug(ts->dev, &ts->debug_reg[0]);
					pr_info(" Screen:%c[%02d]:%s, X=%d, Y=%d, Z=%d, thonum=x%08X, diffsum=x%08X, status=x%08X\n",
						state2char(ts->tdata[i].type),
						i,
						"Up",
						ts->tdata[i].x,
	...
	} else if (ts->debug_mask & TOUCH_KPI_LOG) {
		for (i = 0; i < touch_max_finger(ts); i++) {
			if (new_mask & (1 << i)) {
				if (press_mask & (1 << i)) {
					pr_info("[TP][KPI] Screen:%c[%02d/%02d]:%s, X=%4d, Y=%4d, Z=%3d, time:\t%ld.%06ld\tto\t%ld.%06ld\n",
							state2char(ts->tdata[i].type),
							i,
							ts->tcount,
							"Down",
	...
	} else if (ts->debug_mask & TOUCH_BREAKDOWN_TIME) {
		for (i = 0; i < touch_max_finger(ts); i++) {
			if (new_mask & (1 << i)) {
				if (press_mask & (1 << i)) {
					pr_info("[TP][KPI] Screen:%c[%02d/%02d]:%s, X=%4d, Y=%4d, Z=%3d, time=\t%4ld\tBUS:\t%4ld\tD\n",
							state2char(ts->tdata[i].type),
							i,
							ts->tcount,
							"Down",
					...
					pr_info("[TP][KPI] Screen:%c[%02d/%02d]:%s, X=%4d, Y=%4d, Z=%3d, time=\t%4ld\tBUS:\t%4ld\tM\n",
							state2char(ts->tdata[i].type),
							i,
							ts->tcount,
							"Move",
...
int siw_touch_init_input(void *ts_data)
{
	...
	if (mode_ctrl->support_glove) {
		input_set_abs_params(input, ABS_MT_GLOVE, 0, 1, 0, 0);
	}
