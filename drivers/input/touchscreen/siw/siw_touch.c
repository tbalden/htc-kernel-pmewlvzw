...
#if 0
#define TC_ACK_TIMEOUT 250
static void semaphore_acquire(struct device *dev)
{
	struct siw_ts *ts = to_touch_core(dev);
	struct touch_feature_ctrl *mode_ctrl = &ts->mode_ctrl;
	int ret = 0;

	sema_init(&ts->switch_sensor_hub_sema, 1);
	down(&ts->switch_sensor_hub_sema);
	t_dev_info(dev, "semaphore acquire: %s.\n",
			aod_mode_str[mode_ctrl->aod_mode - FB_AOD_PREPARE_OFF]);
	ret = down_timeout(&ts->switch_sensor_hub_sema,
			(TC_ACK_TIMEOUT * HZ) / 1000);
	if (ret) {
		t_dev_info(dev, "semaphore timeout(%d): %s.\n",
				ret, aod_mode_str[mode_ctrl->aod_mode - FB_AOD_PREPARE_OFF]);
		up(&ts->switch_sensor_hub_sema);
	}
}

void semaphore_release(struct device *dev)
{
	struct siw_ts *ts = to_touch_core(dev);
	struct touch_feature_ctrl *mode_ctrl = &ts->mode_ctrl;

	t_dev_info(dev, "semaphore release: %s.\n",
			aod_mode_str[mode_ctrl->aod_mode - FB_AOD_PREPARE_OFF]);
	up(&ts->switch_sensor_hub_sema);
}
#endif

static void siw_touch_init_method(struct siw_ts *ts);
static void siw_touch_post_pm(struct device *dev, int do_init)
{
	struct siw_ts *ts = to_touch_core(dev);
	struct siw_touch_chip *chip = to_touch_chip(dev);
	struct touch_feature_ctrl *mode_ctrl = &ts->mode_ctrl;

	t_dev_dbg_base(ts->dev, "%s\n", __func__);
	if (do_init) {
		cancel_delayed_work(&ts->init_work);
		siw_touch_init_method(ts);
	}

#if defined(__SIW_CONFIG_FB)
	if (mode_ctrl->prev_aod_mode == mode_ctrl->aod_mode) {
		if (((mode_ctrl->aod_mode == FB_AOD_PARTIAL_ON)
				&& ((chip->prev_lcd_mode == LCD_MODE_U0) || (chip->prev_lcd_mode == LCD_MODE_U3)))
			|| ((mode_ctrl->aod_mode == FB_AOD_IDLE)
				&& ((chip->prev_lcd_mode == LCD_MODE_U2) || (chip->prev_lcd_mode == LCD_MODE_U3)))) {
#if 0
			/* U0->U2, U3->U2 */
			/* U3->U0, U2->U0 */
			semaphore_acquire(dev);
#endif
 ...
 *
 */
void siw_touch_suspend(struct device *dev)
{
	struct siw_ts *ts = to_touch_core(dev);
	int ret = 0;
 ...
 *
 */
void siw_touch_resume(struct device *dev)
{
	struct siw_ts *ts = to_touch_core(dev);
	int ret = 0;
...
static void siw_touch_init_method(struct siw_ts *ts)
{
	...
	if (ts->skip_upgrade) {
		t_dev_info(dev, "Skip touch F/W upgrade.\n");
	} else if (do_fw_upgarde) {
...
#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG)
	if (ts->debug_mask & (TOUCH_KPI_LOG | TOUCH_BREAKDOWN_TIME))
...
static irqreturn_t __used siw_touch_irq_thread(int irq, void *dev_id)
{
	...
	if (mode_ctrl->bus_to_mcu & NANOHUB_TP_SWITCH_MCU_NORMAL) {
		t_dev_info(dev, "[SensorHub] skip irq\n");
		goto out;
	}
#endif
...
static struct siw_touch_pdata *_siw_touch_do_probe_common(struct siw_ts *ts)
{
	...
	/* switch_delay_time */
	mode_ctrl->switch_delay_time[FB_AOD_IDLE] = 20;
	mode_ctrl->switch_delay_time[FB_AOD_PARTIAL_ON] = 20;

	/* switch_delay_time is different under video mode and command mode */

	if (ts->u3_disp_setting == true)
		mode_ctrl->switch_delay_time[FB_AOD_FULL_ON] = 20;
	else
		mode_ctrl->switch_delay_time[FB_AOD_FULL_ON] = 0;

	t_dev_info(dev, "switch_delay_time: %d %d %d\n",
			mode_ctrl->switch_delay_time[FB_AOD_IDLE],
			mode_ctrl->switch_delay_time[FB_AOD_PARTIAL_ON],
			mode_ctrl->switch_delay_time[FB_AOD_FULL_ON]);
...
static int siw_touch_do_probe_normal(void *data)
{
	...
	if (!pdata) {
//		return -EINVAL;
//TODO DEBUG: IC test data unmatch
		return -EPROBE_DEFER;
	}
...
//	BUILD_BUG_ON(sizeof(struct touch_data) != (9<<1));
	...
	if (tamper_flag == 0) {
		ts->debug_mask |= TOUCH_DOWN_UP_LOG;
#if defined(CONFIG_TOUCHSCREEN_HTC_ATTR)
		ts->diag_flag = DIAG_RAW;
		t_dev_info(dev, "debug level=0x%08X, diag flag = %d\n",
				ts->debug_mask, ts->diag_flag);
#else
		t_dev_info(dev, "debug level=0x%08X", ts->debug_mask);
#endif
	}
#endif
	mode_ctrl->aod_mode = FB_AOD_FULL_ON;
	mode_ctrl->prev_aod_mode = FB_AOD_OFF;
	mode_ctrl->tp_bus_sel_en = 1;
	mode_ctrl->bus_to_mcu = 0;
...
//NOTE: Regulator should not drop while enter kernel from lk
//NOTE: MIPI COMMAND SHOULD BE SENT BEFORE PROBE!!!
