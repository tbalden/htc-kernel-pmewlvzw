...
static int siw_touch_spi_init(struct device *dev)
{
	...
	t_dev_info(dev, "spi init: %d hz, mode %d, bpw %d, cs %d (%s)\n",
			spi->max_speed_hz,
			spi->mode,
			spi->bits_per_word,
			spi->chip_select,
...
//	struct siw_ts *ts = spi_get_drvdata(spi);
	...
	struct spi_message m;
	int ret = 0;
#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG)	/* DEBUG */
#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG_SPI)	/* DEBUG */
	int i = 0;
#endif
#endif
	/*
...
#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG)	/* DEBUG */
#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG_SPI)	/* DEBUG */
	printk("[TP] rx, len:%d, %dHz, 0x", x.len, x.speed_hz);
	for(i = 0 ; i < x.len; i++)
		printk("%02X ", *(u8 *)(x.rx_buf + i));
	printk("h\n");
#endif
#endif
	return ret;
...
#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG)	/* DEBUG */
#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG_SPI)	/* DEBUG */
	printk("[TP] tx, len:%d, %dHz, 0x", x.len, x.speed_hz);
	for(i = 0 ; i < x.len; i++)
		printk("%02X ", *(u8 *)(x.tx_buf + i));
	printk("h\n");
#endif
#endif
	return ret;
...
#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG)	/* DEBUG */
#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG_SPI)	/* DEBUG */
	for (j = 0; j < xfer->msg_count; j++) {
		tx = &xfer->data[j].tx;
		rx = &xfer->data[j].rx;

		printk("[TP] TX #:%d/%d, len: %d, data: 0x", j, xfer->msg_count, tx->size);
		for(i = 0 ; i < tx->size; i++)
			printk("%02X ", *(u8 *)(tx->data + i));
		printk("h\n");

		printk("[TP] RX #:%d/%d, len: %d, reg: 0x%04X, data: 0x", j, xfer->msg_count, rx->size, rx->addr);
		for(i = 0 ; i < rx->size; i++)
			printk("%02X ", *(u8 *)(rx->data + i));
		printk("h\n");
	}
#endif
#endif
					...
					struct siw_touch_pdata *pdata)
{
	...
	t_dev_info(dev, "spi alloc: %d hz, mode %d, bpw %d\n",
			tmp, spi->mode, spi->bits_per_word);
