...
void __weak siw_hal_watch_set_disp_waton(struct device *dev, bool value)
{
	t_warn_weak_watch(dev, "%s\n", __func__);
}
...
#define SIW_HAL_GPIO_SWITCH		"siw_hal_switch"
#endif
...
#ifdef CONFIG_NANOHUB_TP_SWITCH
static void siw_hal_set_gpio_switch(struct device *dev, int val)
{
	struct siw_ts *ts = to_touch_core(dev);
...
static void siw_hal_init_gpio_switch(struct device *dev)
{
	struct siw_ts *ts = to_touch_core(dev);
	int switch_pin = touch_switch_pin(ts);
	int ret = 0;
	...
	siw_touch_gpio_direction_output(dev,
			switch_pin, GPIO_OUT_ZERO);
	t_dev_dbg_gpio(dev, "set %s(%d) as output\n",
			SIW_HAL_GPIO_SWITCH, switch_pin);

	siw_touch_gpio_set_pull(dev,
			switch_pin, GPIO_NO_PULL);
	t_dev_dbg_gpio(dev, "set %s(%d) as pull-up(%d)\n",
			SIW_HAL_GPIO_SWITCH,
			switch_pin, GPIO_NO_PULL);
}
...
/*
 * switch_sensor_hub(struct device *dev, int mode)
 *
 * switch bus control pin to CPU/MCU
 * notify MCU with current select pin status and current glove setting
 *
 * NOTE:
 * cover mode status will be trasnmitted to MCU through the registed callback
 * function of MCU.
 */
void switch_sensor_hub(struct device *dev, int mode)
{
	struct siw_ts *ts = to_touch_core(dev);
	...
	if (mode_ctrl->tp_bus_sel_en == 0)
		return;

	mutex_lock(&ts->lock);
	if (mode_ctrl->support_glove){
		mask = (atomic_read(&ts->state.glove) & 0x01) << 1;
	}
...
#if defined(__SIW_SUPPORT_PINCTRL)	//See siw_touch_cfg.h
			if (mode_ctrl->prev_aod_mode == FB_AOD_OFF) {
				if (pinctrl_select_state(pinctrl->ctrl, pinctrl->active))
					t_dev_dbg_gpio(ts->dev, "cannot set pinctrl active\n");
				else
					t_dev_info(ts->dev, "pinctrl set active\n");
			}
#endif
#ifdef CONFIG_NANOHUB_SECOND_DISP
			while (nanohub_is_switch_operating() && (i < NANOHUB_I2C_MAX_WAIT_COUNT)) {
				mdelay(NANOHUB_I2C_DELAY);
				i++;
			}

			if (i)
				t_dev_info(ts->dev, "[SensorHub] waiting count = %d\n", i);
#endif
			siw_hal_set_gpio_switch(dev, mode);
			mode_ctrl->bus_to_mcu = mode;

#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG)
			siw_touch_report_all_event(ts);
#endif
			if (mode_ctrl->prev_aod_mode != FB_AOD_OFF) {
				//For Off->U3, IRQ will be enabled after init done
				siw_touch_irq_control(ts->dev, 1);
			}
			t_dev_info(ts->dev, "[SensorHub] Switch bus to CPU\n");
			break;
		case 1:
		case 8:
		case 9:
			mode_ctrl->bus_to_mcu = mode;
			siw_touch_irq_control(ts->dev, 0);
			siw_hal_set_gpio_switch(dev, mode & NANOHUB_TP_SWITCH_MCU_NORMAL);
#if defined(__SIW_SUPPORT_PINCTRL)	//See siw_touch_cfg.h
			if (mode & NANOHUB_TP_SWITCH_DISABLED) {
				if (pinctrl_select_state(pinctrl->ctrl, pinctrl->suspend))
					t_dev_dbg_gpio(ts->dev, "cannot set pinctrl suspend\n");
				else
					t_dev_info(ts->dev, "pinctrl set suspend\n");
			}
#endif
			t_dev_info(ts->dev, "[SensorHub] Switch bus to %s\n",
					mode & NANOHUB_TP_SWITCH_MCU_NORMAL ? "MCU" : "CPU");
			nanohub_tp_status(mode | mask);
			break;
		default:
			t_dev_err(dev, "%s: incorrect mode %d\n", __func__, mode);
			break;
		}
	} else
		t_dev_err(dev, "%s: invalid switch gpio %d\n", __func__, switch_pin);
	mutex_unlock(&ts->lock);
}

static void siw_hal_free_gpio_switch(struct device *dev)
{
	struct siw_ts *ts = to_touch_core(dev);
	int switch_pin = touch_switch_pin(ts);

	siw_touch_gpio_free(dev, switch_pin);
}
#endif
...
static void siw_hal_fb_notify_work_func(struct work_struct *fb_notify_work)
{
	...
	t_dev_dbg_base(chip->dev, "%s: %s\n", __func__, (chip->lcd_mode == LCD_MODE_U3) ? "FB_RESUME" : "FB_SUSPEND");
...
#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG)
	if (ts->debug_mask == 0) {
		pr_info("[FW] [%s] V%u.%02u(0x%08Xh,%02X)-CID:%s-PID:%s-BOOT:%s(%s)-CRC:%s(0x%08X)\n",
			touch_chip_name(ts),
			fw->v.version.major, fw->v.version.minor,
			version, fw->revision,
			fw->chip_id,
...
extern void siw_touch_suspend(struct device *dev);
extern void siw_touch_resume(struct device *dev);
static void siw_touch_post_fb(struct device *dev, int type)
{
	struct siw_touch_chip *chip = to_touch_chip(dev);
	struct siw_ts *ts = chip->ts;
	struct touch_feature_ctrl *mode_ctrl = &ts->mode_ctrl;
	...
	mode_ctrl->prev_aod_mode = mode_ctrl->aod_mode;

	/* siw_touch_notifier_call_chain(LCD_EVENT_LCD_MODE, (void *)&type);
	 * -> siw_touch_notify
	 * -> _siw_touch_do_notify
	 * -> siw_hal_notify
	 * -> case LCD_EVENT_LCD_MODE
	 */
	mutex_lock(&ts->lock);

	t_dev_info(dev, "%s: lcd_event: lcd mode\n", __func__);
	ret = siw_hal_lcd_mode(dev, type);
	if (ret < 0) {
		siwmon_submit_evt(dev, "NOTIFY", 0, "LCD_MODE", LCD_EVENT_LCD_MODE, type, ret);
		return;
	}
...
#ifdef CONFIG_NANOHUB_TP_SWITCH
	tp_mode = (chip->lcd_mode | (chip->driving_mode << 3)) | (0x1 << 6);
	t_dev_info(dev, "[SensorHub] lcd mode: 0x%x tc_driving mode:0x%x\n",
			chip->lcd_mode, chip->driving_mode);
	nanohub_tp_mode(tp_mode);
#endif
	siwmon_submit_evt(dev, "NOTIFY", 0, "LCD_MODE", LCD_EVENT_LCD_MODE, type, 0);

	if (!ret) {
		/* chip->fb_notify_work */
		type = (chip->lcd_mode == LCD_MODE_U3) ? FB_RESUME : FB_SUSPEND;
		t_dev_dbg_base(dev, "%s: %s\n", __func__,
				(chip->lcd_mode == LCD_MODE_U3) ?
				"FB_RESUME" : "FB_SUSPEND");
		 ...
		 * -> case NOTIFY_FB
		 */
		atomic_set(&ts->state.fb, type);
		t_dev_info(dev, "%s: event %s(%Xh), value %Xh\n",
			__func__, "FB", (int)NOTIFY_FB, type);
		siwmon_submit_evt(dev, "NOTIFY", 0, "FB", NOTIFY_FB, type, 0);
		mutex_unlock(&ts->lock);

		/* siw_touch_qd_fb_work_now -> ts->fb_work */
		...
		unsigned long event, void *data)
{
	...
	struct fb_event *ev = (struct fb_event *)data;
	int type = 0;
	uint32_t lcdmode[4] = {LCD_MODE_MAX + 1, LCD_MODE_U0, LCD_MODE_U2, LCD_MODE_U3};

	t_dev_dbg_base(dev, "event=%lu\n", event);
	mutex_lock(&ts->fb_lock);
	if (ev && ev->data && event == FB_EARLY_EVENT_BLANK) {
		int *blank = (int *)ev->data;

		t_dev_info(ts->dev, "event=%lu, data=%d\n", event, *blank);
		goto out;

	} else if (ev && ev->data && event == FB_EVENT_AOD_MODE) {
		int *blank = (int *)ev->data;

		t_dev_info(ts->dev, "event=%lu, data=%d\n", event, *blank);
		if ((*blank >= FB_AOD_PREPARE_OFF) && (*blank < FB_AOD_MAX)) {
			mode_ctrl->aod_mode = *blank;
			t_dev_info(ts->dev, "[AOD] %s->%s\n",
				aod_mode_str[mode_ctrl->prev_aod_mode - FB_AOD_PREPARE_OFF],
				aod_mode_str[mode_ctrl->aod_mode - FB_AOD_PREPARE_OFF]);
		} else {
			t_dev_err(dev, "[AOD] invalid AOD mode\n");
			goto out;
		}

		if ((mode_ctrl->aod_mode == FB_AOD_PREPARE_OFF)
			&& ((mode_ctrl->prev_aod_mode != FB_AOD_PARTIAL_ON) || (mode_ctrl->prev_aod_mode != FB_AOD_IDLE))) {
			/* U3->OffP, Off->OffP */
			/* When U3->Off, we get 2 FB_EVENT_AOD_MODE event: U3->OffP, OffP->Off */

#if defined(CONFIG_NANOHUB_TP_SWITCH)
			/* Switch bus to CPU & set pinctrl suspend: for U3->Off */
			switch_sensor_hub(ts->dev, 8);
#endif
			mode_ctrl->prev_aod_mode = mode_ctrl->aod_mode;

		} else if ((mode_ctrl->aod_mode == FB_AOD_OFF)
				&& (mode_ctrl->prev_aod_mode == FB_AOD_PREPARE_OFF)) {
			/* OffP->Off */
			mode_ctrl->prev_aod_mode = mode_ctrl->aod_mode;
		}

	} else if (ev && ev->data && event == FB_EVENT_BLANK) {
		int *blank = (int *)ev->data;

		t_dev_info(ts->dev, "event=%lu, data=%d\n", event, *blank);
		/* No U0->U0: no FB_EVENT_AOD_MODE event from display driver since AOD modes are the same. */
		if (((mode_ctrl->prev_aod_mode == mode_ctrl->aod_mode) && (mode_ctrl->aod_mode != FB_AOD_PARTIAL_ON))
				|| ((mode_ctrl->prev_aod_mode == FB_AOD_OFF) && (mode_ctrl->aod_mode != FB_AOD_FULL_ON))
				|| (mode_ctrl->aod_mode == FB_AOD_OFF)) {
			/* Invalid Scenario:
			 * U3->U3, U0->U0, OFF->OFF, OFF->U0, OFF->U2, U0->OFF, U2->OFF
			 *
			 * Already handled at event == FB_EVENT_AOD_MODE:
			 * U3->OFF
			 */
		} else if ((mode_ctrl->aod_mode == mode_ctrl->prev_aod_mode) && (mode_ctrl->aod_mode == FB_AOD_PARTIAL_ON)) {
			/* U2->U2 (watch setup):
			 * Since AOD modes are the same, no FB_EVENT_AOD_MODE event from display driver
			 */
			t_dev_info(ts->dev, "[AOD] %s->%s (%s)\n",
				aod_mode_str[mode_ctrl->prev_aod_mode - FB_AOD_PREPARE_OFF],
				aod_mode_str[mode_ctrl->aod_mode - FB_AOD_PREPARE_OFF],
				*blank == FB_BLANK_POWERDOWN ? "Freeze" : "Melt");

			if (*blank == FB_BLANK_POWERDOWN) {
#if defined(CONFIG_NANOHUB_TP_SWITCH)
				/* Switch bus to CPU */
				switch_sensor_hub(ts->dev, 0);
#endif

				if (siw_hal_watch_is_disp_waton(dev)) {
					siw_hal_watch_font_update(dev);
					siw_hal_watch_font_onoff(dev, true);
				}

#if defined(CONFIG_NANOHUB_TP_SWITCH)
				/* Switch bus to MCU */
				switch_sensor_hub(ts->dev, 1);
#endif
			}
		} else {
			/* U2->U3, U0->U3, Off->U3, U3->U2, U3->U0, U2->U0, U0->U2 */
			if (mode_ctrl->prev_aod_mode == FB_AOD_OFF) {
				/* Off->U3 */
#if defined(CONFIG_NANOHUB_TP_SWITCH)
				/* Switch bus to CPU & set pinctrl active */
				switch_sensor_hub(ts->dev, 0);
#endif
				siw_touch_notifier_call_chain(LCD_EVENT_HW_RESET, NULL);
				mdelay(60);
			} else if (((mode_ctrl->prev_aod_mode == FB_AOD_IDLE) && (mode_ctrl->aod_mode == FB_AOD_PARTIAL_ON))
				|| ((mode_ctrl->prev_aod_mode == FB_AOD_PARTIAL_ON) && (mode_ctrl->aod_mode == FB_AOD_IDLE))
				|| ((mode_ctrl->prev_aod_mode != FB_AOD_OFF) && (mode_ctrl->aod_mode == FB_AOD_FULL_ON))) {
				/* U2<->U0, U0->U3, U2->U3 */

				t_dev_info(ts->dev, "delay %d\n", mode_ctrl->switch_delay_time[mode_ctrl->aod_mode]);
				mdelay(mode_ctrl->switch_delay_time[mode_ctrl->aod_mode]);
			} else if ((mode_ctrl->prev_aod_mode == FB_AOD_FULL_ON) && mode_ctrl->aod_mode == FB_AOD_PARTIAL_ON) {
				/* U3->U2 */
				if (*blank == FB_BLANK_POWERDOWN) {
					if (siw_hal_watch_is_disp_waton(dev)) {
						siw_hal_watch_font_update(dev);
			...
			/* Notify mode change */
			type = lcdmode[mode_ctrl->aod_mode];
			siw_touch_post_fb(dev, type);
		}
	}
out:
	mutex_unlock(&ts->fb_lock);
	return 0;
#endif	/* CONFIG_TOUCHSCREEN_HTC_DEBUG */
}
...
#else
	if (mode_ctrl->charger_setup_skip) {
		siw_hal_write_value(dev,
				reg->spr_charger_status,
				chip->charger);
		mode_ctrl->charger_setup_skip = false;
...
#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG)
		/* update firmware if the following condition is true:
		 * 1. versions are different
		 */
		if ((bin_major != dev_major) || (bin_minor != dev_minor)) {
			update |= (1<<0);
		}
#else
				...
				struct device *dev)
{
	...
	if (ts->test_fwpath[0]) {
		...
		if (((ts->u3_disp_setting << 1) | ts->eng_id) < ts->def_fwcnt)
			src_path = (char *)ts->def_fwpath[(ts->u3_disp_setting << 1) | ts->eng_id];
		else
			src_path = (char *)ts->def_fwpath[(ts->u3_disp_setting << 1)];
	} else {
...
static int siw_hal_tc_driving(struct device *dev, int mode)
{
	...
	if (mode == LCD_MODE_STOP) {
		...
		if ((!running_status) && ((u32)mode != rdata))
			if (mode_ctrl->switch_delay_time[mode_ctrl->aod_mode] <= SWITCH_DELAY_TIME_MAX)
				mode_ctrl->switch_delay_time[mode_ctrl->aod_mode] += 5;
	}

	if (re_init) {
#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG)
		t_dev_err(dev, "mode: %s, disp[%04Xh]:0x%08X, TC Driving[%04Xh] wr 0x%08X\n",
			siw_lcd_driving_mode_str(mode),
			reg->spr_subdisp_status, rdata,
			reg->tc_drive_ctl, ctrl);
#endif
...
#if 0
extern void semaphore_release(struct device *dev);
#endif
...
#if 0
#if defined(__SIW_CONFIG_FB)
	else if ((ret == -ERANGE) && (mode_ctrl->prev_aod_mode == mode_ctrl->aod_mode)) {
		if ((mode_ctrl->aod_mode == FB_AOD_PARTIAL_ON)
				&& ((chip->prev_lcd_mode == LCD_MODE_U0) || (chip->prev_lcd_mode == LCD_MODE_U3))) {
			/* U3->U2, U0->U2 */
			semaphore_release(dev);
		} else if ((mode_ctrl->aod_mode == FB_AOD_IDLE)
				&& ((chip->prev_lcd_mode == LCD_MODE_U2) || (chip->prev_lcd_mode == LCD_MODE_U3))) {
			/* U3->U0, U2->U0 */
			semaphore_release(dev);
		}
	}
...
static int siw_hal_irq_handler(struct device *dev)
{
...
#endif
#if defined(CONFIG_TOUCHSCREEN_HTC_DEBUG)
	if (ts->debug_mask & TOUCH_BREAKDOWN_TIME)
		getnstimeofday(&ts->tp_read_start_time);
	if (ts->debug_mask & TOUCH_BREAKDOWN_LOG)
		pr_info("[TP][KPI] %s: siw_hal_reg_read START\n", __func__);
#endif
	ret = siw_hal_reg_read(dev,
				reg->tc_ic_status,
	...
	if (ts->debug_mask & TOUCH_BREAKDOWN_LOG)
		pr_info("[TP][KPI] %s: siw_hal_reg_read DONE\n", __func__);
#endif
#if defined(__SIW_SUPPORT_PM_QOS)
	...
	if (chip->info.wakeup_type == ABS_MODE) {
		...
		if (ret) {
			t_dev_err(dev, "siw_hal_irq_abs failed, %d\n", ret);
			goto out;
		}
	} else {
...
static void siw_hal_connect(struct device *dev)
{
	...
	if (mode_ctrl->bus_to_mcu & NANOHUB_TP_SWITCH_MCU_NORMAL) {
		mode_ctrl->charger_setup_skip = true;
		t_dev_info(dev, "charger_state set skip\n");
	} else {
		siw_hal_write_value(dev,
				reg->spr_charger_status,
				chip->charger);
...
#ifdef CONFIG_NANOHUB_TP_SWITCH
		tp_mode = (chip->lcd_mode | (chip->driving_mode << 3)) | (0x1 << 6);
		t_dev_info(dev, "[SensorHub] Signal lcd mode: 0x%X tc_driving mode:0x%X\n",
				chip->lcd_mode, chip->driving_mode);
		nanohub_tp_mode(tp_mode);
#endif
